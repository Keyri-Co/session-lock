...
Session tokens are a weak link in account security, as attackers can steal them to bypass all authentication and access protected resources. [Existing](https://datatracker.ietf.org/doc/rfc8471/) [RFCs](https://datatracker.ietf.org/doc/rfc8472/) [have](https://datatracker.ietf.org/doc/rfc8473/) [attempted](https://datatracker.ietf.org/doc/rfc6819/) to address the token hijacking threat, but they are not widely adopted (only available on Edge) and their solutions can be impractical (targeting the networking layer). A draft RFC for [demonstrating proof-of-possession (DPoP)](https://datatracker.ietf.org/doc/draft-ietf-oauth-dpop/16/) proposes a similar mechanism to session-lock, but it is only applicable to OAuth systems and complex to implement.

Here's a manual demonstration of how session hijacking works. We're taking the the token from a logged-in session and using it on another browser. A bad actor would do this programmatically with a browser extension or malware, but the principle is the same.
...
Anatomy of a regular JWT: \`{header}.{payload}.{server sig}\`

Anatomy of a session-lock token: \`{header}.{payload w/ client pubKey}.{server sig}.{timestamp}.{client sig}\`
